...
int partition2(struct BoxData arr[], int low, int high)
{
    //always select pivot as rightmost element
    struct BoxData pivot = arr[high];
...
void quicksort2(struct BoxData arr[], int low, int high)
{
    //"domain" check
    if (low < high)
    {
...
void DFS_Visit(struct BoxData graph[], int vertex, int *time, int counter, int *index, struct BoxSelect selected_boxes[], int *max_connected_nodes)
{
    (*time)++;
...
void DFS(struct BoxData graph[], int num_vertices, int *index, struct BoxSelect selected_boxes[], int *max_connected_nodes)
{
...
*/
    ...
    quicksort2(box_data, 0, total - 1);

    //? DEBUG: prints this newly sorted array
    for (int i = 0; i < total; i++)
    {
        printf("\nNo %3d    W % 5.1f   H %5.1f   D %5.1f    V %8.1f    EndTime %d\n", box_data[i].number, box_data[i].width, box_data[i].height, box_data[i].depth, box_data[i].volume, box_data[i].end_time);
    }

    //getting longest list of stacked boxes
    for (int i = total - 1; i >= 0; i--)    //starts from last box in box_data_top_sort and goes backwards
    {
        int index = 0; //index counter, reset on every iteration

        //always starts adding current box
        current_selection[index].box_pos = i;
        current_selection[index].number = box_data[i].number;

        //checking backwards
        for (int j = i - 1; j >= 0; j--)
        {
            struct AdjacencyList *temp = box_data[j].head_adjacency_list;

            //run through adjacency list to find a link to currently selected box
            while (temp != NULL)
            {
                //if preceding box has a link to currently selected box
                if(temp->number == current_selection[index].number)
                {
                    index++;
                    current_selection[index].box_pos = j;
                    current_selection[index].number = box_data[j].number;
                    break;
                }
        ...
        //copies values if current link is greater than former max link
        index++;
        if (index>max_connected_nodes)
        {
            max_connected_nodes = index;
            memcpy(&selected_boxes, &current_selection, sizeof(current_selection));
        }
    }
    ...
    {
        printf("\nBOX No %3d - index %d", selected_boxes[i].number, selected_boxes[i].box_pos);
    }
