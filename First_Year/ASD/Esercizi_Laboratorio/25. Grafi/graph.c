...
{
    /* Prestare attenzione al fatto che se il grafo è non orientato,
       per inserire l'arco (src, dst) occorre sia inserire dst nella
       lista di adiacenza di src, sia inserire src nella lista di
       adiacenza di dst.

       Per risparmiare tempo è possibile assumere che l'arco non
       esista già. La soluzione che verrà fornita effettua questo
       controllo per maggiore robustezza, ma cio' peggiora il costo
       asintotico dell'operazione. */
