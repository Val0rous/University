...
#define MAX_APPOINTMENTS (366 * 2)     //average of 2 appointments for each day
#define clear() printf("\e[1;1H\e[2J") //alternatively

//clear screen instruction in different OS
#ifdef _WIN32
#define CLEAR "cls"
...
int main()
{
    ...
    while (1) //MAIN CYCLE
    {
        ...
        if (answer == '1')
        {
            system(CLEAR);
            //Insert description
            printf("Which is the description?\n");
            ...
            while (1)
            {
                ...
                //that is, if the start time is legal
            ...
            while (1)
            {
                while (1)
                {
                    ...
                    //That is, if the end time is legal
                ...
                //If user is not so savvy and wants to create an event with either not acceptable times or total length equal to zero
            ...
            while (cursor != NULL) //checks each element
            {
                ...
                {
                    //convert times in hh:mm format, easier to manipulate
        ...
        if (answer == '3')
        {
            ...
            {
                printf("Which month do you want to print? (It's in the order you have inserted)\n");
            ...
            if (results > 0)
                printf(" %d result%c press any key to continue (Except the power button)\n", results, results > 1 ? 's' : '\0');
    ...
    //free heap memory for each item
    cursor = first;
